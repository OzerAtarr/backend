      ...
      try {
        ...
        if (user) {
          ...
          let privileges = rolePrivileges.map((rp) =>
            privs.privileges.find((x) => x.key == rp.permissions)
          );
  ...
  return {
    ...
    checkRoles: (...expectedRoles) => {
      return (req, res, next) => {
        ...
        while (i < exitOnError.length && !privileges.includes(expectedRoles[i]))
          i++;
        ...
        return next(); // Authorize kısmına geçecek yetkileri var
      };
    },
  };
};
